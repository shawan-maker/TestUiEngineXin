...
这些是多步骤组合的高级关键字，一个调用 = 多个操作步骤：

| 关键字 | Excel 写法示例 | 功能说明 |
|--------|---------------|---------|
| 等待加载完成 | `等待加载完成` | 等待 3 种 loading 元素全部消失 + 1s 稳定等待 |
| 列表查询 | `列表查询(项目名称, 测试项目A)` | 在搜索框输入关键词 → 点击查询 → 等待加载完成 |
| 导出验证 | `导出验证` | 点击导出 → 等待下载 → 验证文件存在 |
| 检查站内信显示 | `检查站内信显示(任务提醒)` | 点击 tab → 获取消息数 → 数量>0 则查看详情并断言标题一致 |
| 首页列表校验 | `首页列表校验(待办事项)` | 获取指定区域列表行数 → 数量>0 则点击第一条链接 |
...
AI 会自动执行全部阶段（脚手架生成 → 元素探测 → 脚本生成 → 验证），你只需等待完成。
