...
# 0. 切换到 tools/ 目录（fixers/ 在 tools/ 下）
...
if phase_1b:
    print('  OK defined')
    if phase_1b.get('tool') == 'excel/read_excel.py':
        print('  OK tool')
    else:
...
with open('verification/verify_orchestrator.py', encoding='utf-8') as f:
    vl_content = f.read()
if 'truly_unresolved' in vl_content and 'kb_fallback_stored' in vl_content:
    print('  OK')
else:
    errors.append('verify_orchestrator missing new exit logic')

# 9. Suite keyword
print('\n9. Suite keyword')
with open('generators/generate_suites.py', encoding='utf-8') as f:
    gs_content = f.read()
...
print('\n14. compile_module_keywords path')
with open('generators/compile_module_keywords.py', encoding='utf-8') as f:
    cmk_content = f.read()
