...
| 5c | `点击第N条记录的XX` | `点击第一条记录的名称`（跳转详情页） |
...
| 14h | `检查XX与YY一致` | `检查消息详情标题与消息列表标题一致` |
...
| 编号 | 格式 | 示例 |
|------|------|------|
| 17a | `访问 URL` | `访问 http://example.com/page` |
| 17b | `等待XX加载完成` | `等待页面加载完成` / `等待数据出现` / `等待列表展示` |
| 17c | `等待进入/弹出/出现XX` | `等待进入工单详情页面` |
| 17d | `等待Ns` | `等待3s` |
| 17e | `返回` | `返回` |
| 17f | `刷新` | `刷新` |
| 17g | `弹窗确认"XX"` | `弹窗确认"确定要删除吗？"` |
| 17h | `确认删除...点击"XX"` | `确认删除并点击"确定"` |
| 17i | `请完成测试结论` | `请完成测试结论` |
...
| 访问无 URL | `访问问题列表页` | `访问 http://100.71.19.25:30101/question/list` | 必须使用完整 URL，自然语言无法解析 |
