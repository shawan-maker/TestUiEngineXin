...
_RAW_PATTERNS = [
    ...
    # ── 随机手机号/邮箱系列（在 随机名称 之前，更短更具体，优先命中）──
    # 在"手机号"输入框中输入随机手机号
    (rf'在{Q}(.+?){Q}\s*(?:输入框|文本框|框)中?\s*(?:输入|填入)随机手机号',
     'fill', ('field',)),
    # 在"邮箱"输入框中输入随机邮箱
    (rf'在{Q}(.+?){Q}\s*(?:输入框|文本框|框)中?\s*(?:输入|填入)随机邮箱',
     'fill', ('field',)),
    ...
    # ── el-cascader 系列（4 个，含序号变体）──
    # Pattern 1a: 在"字段"第N个级联选择框中依次选择"值1"、"值2"（带序号）
    (rf'在{Q}(.+?){Q}\s*第[一二三四五六七八九十\d]+个?(?:级联选择器|级联选择框|级联框|级联)中(?:依次)?(?:选择|勾选)(.*)',
     'el_cascader', ('field', 'values_raw')),
    # Pattern 1b: 在"字段"级联选择框中依次选择"值1"、"值2"（无序号）
    ...
    # 点击(第N条记录的)"更多"(按钮)，选择/点击"XX"（复合步骤：展开+操作，行号前缀可选）
    (rf'(?:点击第[一二三四五六七八九十\d]+条(?:查询)?记录的)?{Q}更多{Q}(?:按钮)?[，,]?\s*(?:选择|点击)\s*{Q}(.+?){Q}',
    ...
    # 支持"或"关系：断言第一条记录的协议类型包含"IPv6+IPv4"或"IPv4+IPv6"
    # 非贪婪捕获到最后一个闭合引号，尾部断言关键字（可见/显示等）不纳入值
    (rf'(?:断言[：:]?|查看|检查|确认).*?(?:第一条|第一行).*?{Q}(.+?){Q}(?:\s*(?:可见|显示|存在|展示|出现))?\s*$',
     'assert_row', ('value',)),
...
def list_patterns_for_prompt():
    ...
    type_examples = {
        ...
        'click_more_then': ['点击"更多"，选择"编辑"', '点击第一条记录的"更多"按钮，点击"删除"'],
